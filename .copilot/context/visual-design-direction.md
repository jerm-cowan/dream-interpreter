# Visual Design Direction — Dream Exploration
...
- **Lens-toggle loading status:** selection/lines already update instantly on tap, so there's no full-screen block — but while the debounced `api/synthesize` call is in flight, the toggle-instruction copy beneath the tone control is swapped for a "Generating new responses..." message with a small spinning icon, reverting once the response lands.
...
- **Implemented:** [Fraunces](https://fonts.google.com/specimen/Fraunces) (expressive display serif) as `font-display`, applied to the app name ("Dream Exploration"), the per-screen page titles ("Choose your lenses", "Dream Analysis"), and each lens's heading/label — not used for general body copy.
...
- The Results screen no longer has a separate row of lens filter chips. Each lens's `InterpretationCard` is itself the toggle, reusing the same gemstone selected/unselected treatment as the Constellation screen so the interaction language stays consistent across both screens.
- **Hover affordance:** hovering a lens card thickens its border via an inset ring (box-shadow), not an actual border-width change — this signals interactivity without ever shifting internal content/layout.
...
- **Wide breakpoints (tablet-landscape and up):** the three cards sit side-by-side in a row, each with a fixed height (400px) and its own hover-revealed scrollbar (thin, rounded, invisible until hover) if its content overflows — this keeps all three cards' heights aligned for easy scanning even when content length varies drastically, rather than letting one lens's card dwarf the others. The gemstone-tinted `LensIcon` + label header stays as the click target at this breakpoint, unchanged from the original single-card treatment. Connecting lines draw **downward** from each selected card to a single point at the top of the synthesis container below: the two outer lenses (Psychology, Symbolism & Culture) step down/across/down in an elbow shape, while the middle lens (Neuroscience) drops in a straight line, since it's already vertically aligned with the synthesis container's center.
- **Mobile/tablet-portrait:** each card becomes an accordion. The icon is dropped at this breakpoint; the header instead shows a selection checkbox + lens name on the left and an expand/collapse caret on the right — only the checkbox toggles selection, only the caret toggles the body's visibility, and neither action is reachable by tapping elsewhere in the header. All cards default to expanded on arrival at Results, regardless of carried-over selection state.
- The synthesis container mirrors this: side-by-side columns (with a vertical divider) at wide breakpoints, stacked with a horizontal divider on mobile/tablet-portrait — and its own caret-only accordion (no selection checkbox, since it's a computed result, not a selectable input) at the narrower breakpoints.
